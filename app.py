...
def update_appliance_settings():
    try:
        ...
        appliance['relay_number'] = int(new_relay_number)
        save_user_data(user_data)
...
def get_analytics():
    ...
    hourly_data_list = [entry for entry in data if datetime.strptime(entry['date'], '%Y-%m-%d') >= now - timedelta(days=1)]
    hourly_data = {f"{entry['date']} {entry['hour']:02d}:00": entry['consumption'] for entry in hourly_data_list}
    
    # Process data for daily view (last 7 days)
    daily_data = {}
    last_week_data = [entry for entry in data if datetime.strptime(entry['date'], '%Y-%m-%d') >= now - timedelta(days=7)]
    for entry in last_week_data:
        daily_data.setdefault(entry['date'], 0)
        daily_data[entry['date']] += entry['consumption']
    
    # Process data for monthly view (last 12 months)
    monthly_data = {}
    last_year_data = [entry for entry in data if datetime.strptime(entry['date'], '%Y-%m-%d') >= now - timedelta(days=365)]
